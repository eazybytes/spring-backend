...
const TagPage = ({ onTaskSelect, onAddNewTask }) => {
  const { tagName } = useParams();
  const { getTasksByTag, lists, tags } = useTodo();
  
  const tasks = tagName ? getTasksByTag(decodeURIComponent(tagName)) : [];
  const tag = tags.find(t => t.name === decodeURIComponent(tagName || ''));
  ...
  if (!tagName || !tag) {
    return (
      <div className="p-4 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400">The requested tag does not exist.</p>
      </div>
    );
  }
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center space-x-4 mb-4">
          <div
            className="px-6 py-3 rounded-2xl text-white text-lg font-bold shadow-lg border border-white/20 backdrop-blur-sm"
      ...
      <div>
        {tasks.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-lg">No tasks with {tag.name} tag. Time to relax or create something new.</p>
          </div>
        ) : (
